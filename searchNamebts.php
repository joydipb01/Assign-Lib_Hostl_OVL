...
<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        $title = trim($_POST["bname"]);
        $query = mysqli_query($conn, "SELECT * FROM BOOKS WHERE BNAME LIKE '%$title%'");
		...
		if(mysqli_num_rows($query)>=1){
            ...
            echo '  <script> alert("No Book named '. $title.' exists");  window.location.href = "./searchName.php"</script>';
        }
    }
